...
export class FindAccountPage extends React.Component {
  ...
  render() {
    ...
    const {sixthTimes} = this.state
    const {isEditTel} = this.state
    ...
    <ScrollView style={styles.pageBG}
      ...
      keyboardShouldPersistTaps='never'>
        ...
        <View>
          ...
          <View style={styles.inputsText}>
            <TextInput secureTextEntry={false}  placeholder="请输入验证码"/>
          </View>
          <View style={styles.inputsText}>
            <TextInput  defaultValue={this.state.newPwd} secureTextEntry placeholder="请输入新密码" onChangeText={this._newPwdChange}/>
          </View>
          <View style={styles.inputsText}>
            {/*<TextInput clearButtonMode='always' defaultValue={this.state.reNewPwd} secureTextEntry placeholder="请确认新密码" onChangeText={this._reNewPwdChange} onBlur={this._reNewPwdBlur}/>*/}
            {ReNewPwdView}
            ...
            <Modal animationType='fade' presentationStyle='formSheet' visible={this.state.modalVisible}
              ...
              <View style={styles.containerModal}>
                <View style={{width:width*0.8,height:height*0.6,backgroundColor:'#fff',justifyContent:'center',alignItems:'center'}}>
                  ...
                  <TouchableHighlight style={{flex:1}} activeOpacity={1} onPress={()=>{this._setModalVisible(!this.state.modalVisible)}}>
                    <LinearGradient colors={['#09B6FD', '#6078EA']} start={{x:0,y:0}} end={{x:1,y:0}} style={[styles.goBackLogin,{width:width*0.5}]}>
                      <Text  style={{fontSize:20,color:'#fff'}} onPress={this._goBackLogin}>登录</Text>
                    </LinearGradient>
                  </TouchableHighlight>
                </View>
              </View>
  ...
  //点击确定按钮
  _findPassword = async () => {
    // this.props.navigation.navigate('Auth'); // 找回密码(进入登陆页面)
    //修改成功后出现模态框
